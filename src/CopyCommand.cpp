...
void CopyCommand::execute(const std::vector<std::string>& args) const {
    // smcli copy <path> to <path> [as <name>]
    if (args.size() < 3 || args[1] != "to") {
        std::cerr << "Usage: smcli copy <source> to <destination> [as <new_name>]" << std::endl;
        return;
    }
    ...
    try {
        ...
        if (fs::exists(final_destination)) {
            if (new_name.empty()) { // Only do conflict resolution if a name wasn't explicitly provided
                ...
                std::cout << "Destination '" << (destination_path / source_path.filename()) << "' already exists. Copying to '" << final_destination.filename() << "'." << std::endl;
            } else {
        ...
        std::cout << "Successfully copied '" << source_path << "' to '" << final_destination << "'." << std::endl;
